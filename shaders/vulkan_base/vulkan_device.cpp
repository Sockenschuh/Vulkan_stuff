...
bool initVulkanInstance(VulkanContext* context, uint32_t instanceExtensionCount, const char** instanceExtensions) {
	...
	validationFeatures.enabledValidationFeatureCount = 0;//ARRAY_COUNT(enableValidationFeatures);
	validationFeatures.pEnabledValidationFeatures = 0;// enableValidationFeatures;
...
bool createLogicalDevice(VulkanContext* context, uint32_t deviceExtensionCount, const char** deviceExtensions) {
	...
	if (computeQueueIndex == graphicsQueueIndex && computeQueueIndex < (numQueueFamilies - 1)) {
		for (uint32_t i = computeQueueIndex + 1; i < numQueueFamilies; ++i) {
			VkQueueFamilyProperties queueFamily = queueFamilies[i];
			if (queueFamily.queueCount > 0) {
				if (queueFamily.queueFlags & VK_QUEUE_COMPUTE_BIT) {
					computeQueueIndex = i;
	...
	}
	{
		...
		// Acquire queue´for compute
		context->computeQueue.familyIndex = computeQueueIndex;
		VK(vkGetDeviceQueue(context->device, computeQueueIndex, 0, &context->computeQueue.queue));
	}
